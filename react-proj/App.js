...
async function getDataAxios(msg){
    const response =
        await axios.get("https://murmuring-hollows-60548.herokuapp.com/message/"+msg)
...
export default class ChatView extends Component {
    ...
    constructor(props) {
        ...
        var loremIpsum = ['Hey i am Winnie. Whats up?', 'Hi friend. How are you doing?', 'Hey long time no see. Whats up?', 'Hello dear, how are you today?']
    ...
    _sendMessage() {
        ...
        getDataAxios(this.state.inputBarText).then(data=>{
            this.state.messages.push({direction: "left", text: data})
            this.setState({
                messages: this.state.messages,
                inputBarText: ''
            })

        }).catch(e=>this.state.messages.push({direction: "left", text: "Sorry, Can't resolve request"}))
